...
def main():
    ...
    if uploaded_files:
        ...
        # Expander com todos os PDFs
        with st.expander("Todos os PDFs"):
            for idx, df_data in enumerate(data):
                st.subheader(f"PDF {idx+1}")
                df = pd.DataFrame(df_data, columns=["Item", "Descrição", "Qtde.", "Unid.", "Vl. unid.", "Vl. total"])
        ...
        with st.expander("Total com Estatísticas e Gráficos"):
            ...
            st.write(f"Número Total de Itens em todos os PDFs: {sum(len(df) - 1 for df in data)}")
        ...
        if excel_file:
            ...
            writer.close()  # Salvar e fechar o arquivo Excel
